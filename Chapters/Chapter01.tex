...
Other applications may include, Temporal induced disability, Neuroergonomy, early detection of errors, affective computing, biometric authentication, teleprescence (improvement of haptic interface), ciberinfrastructure and assistive robotics.  Intensive care units and disorders of consciousness \cite{Annen2018} (detection of remanent activity in comatose patients) are recent disciplines where BCI is showing tremendous prospects and possible applications.  

Nowadays Stroke Rehabilitation is the only area where clinical trials for BCI are being conducted. The important aspect here is that it is known that the neurofeedback provided by a BCI interface provides a significant increase in motor rehabilitation (GUAN CUNTAI NTU paper).
