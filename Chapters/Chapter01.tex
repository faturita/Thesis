...
\textit{"We yet have an impractical and inaccessible exotica for very specific user groups"\cite{Allison2010}}

\textit{"Effectiveness of non-invasive BCI systems remain limited…"\cite{Wolpaw and Wolpaw2011}}

\textit{"…to ponder if BCIs are really promising and helpful, or if they are simple a passing rod, reinforced by their sci-fi side..."\cite{Lotte2016}}

Overall, as the feasibility of the system has been proved current challenges in BCI can be summarizing at 
Increasing ITR, SNR, Reliability, Portability, UX, Out-of-lab-ization and Decreasing  Setup, Trainning and,Calibration Time and
Inter/Intra variability.

\section{Taxonomy}

From the signal processing point of view, BCIs are:

\begin{itemize}
\item Causal
\item Dynamic
\item Time invariant (adaptative with calibration are not)
\item Non linear
\item Multi rate
\end{itemize}
...
\begin{figure}[]
\centering
\includegraphics[scale=1]{images/neuroanatomy.png}
\caption{Sample EEG signal obtained from (g.Nautilus, g.Tec, Austria)  Time axis is in seconds and five seconds are displayed.  The eight channels provided by this device are shown.}
\label{fig:sampleeeg}
\end{figure}
...
Independent or asynchronous or self-paced
Dependant hybrid BCI (gaze)

The discipline stepped forward toward pBCI.

Feedback, mutual adaptive controllers.  Continual Adaptation and zero calibration.

Hybrid BCI

Spatial Filter
\begin{equation}
y(n) = M T\left[x(n)\right]
\end{equation}

Static Filter: square or logartihmic.
...
Bereitschaftspotentials, Readiness Potential or Movement-Related Potentials

Motor Imagery
ERD/ERS: Event Related
Wadsworth BCI
Graz BCI
...
Focusing on EEG Noninvasive BCI
...
\subsection{Volumetric Conduction}
