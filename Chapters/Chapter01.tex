...
\begin{itemize}
\item Causal  $ y(n) = f( \mathbf{v}(m) ) $, where $ m \leq n $.
\item Dynamic: it is not a function of only the current value $ y(n) = f( \mathbf{v}(m) ) $.
...
\item Non linear: the system is linear when $T\left[ a_1 \mathbf{v}(n) + a_2 \mathbf{v}(n) \right]  = a_1 T \left[ \mathbf{v}(n) \right] + a_2 T \left[ \mathbf{v}(n) \right] $ 
\item Multi rate: or broadband.  The energy of the FFT is not confined to a certain band, and almost all frequency channels convey some information.
\end{itemize}
