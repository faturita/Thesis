...
\begin{chronology}[5]{1966}{2014}{80ex}[\textwidth]
...
A noteworthy aspect is that Stroke Rehabilitation is the only area where clinical trials for BCI are being conducted. It is understood that the neurofeedback provided by a BCI interface improves the prognosis of motor rehabilitation \cite{Ang2011}.
...
\item Dynamic: $ y(n) = f( \mathbf{v}(m),  \mathbf{\dot{v}}(m),\mathbf{\ddot{v}}(m)) $.  A BCI system is dynamic, where the output function do not depend only on the current value being observed, it does depend on the dynamic interactions.
...
\item Multirate or broadband \cite{Miller2010}.  The energy of brainwaves spectrum is not confined to a certain band, and almost all frequency channels convey some information.
\end{itemize}
...
\textbf{Static Filters} like square or logartihmic were traditionally used in analog signal processing and are currently part of many of the brainwave measuring devices.  Wiener and Kallman Filters are usually applied to invasive techniques.  The filter, particularly when it is linear, can be viewed as:

\begin{equation}
y(n) = M T\left[\mathbf{x}(n)\right]
\end{equation}

\textbf{Spatial filters} emphasize the arrangement of sensors around or within the head and the spatial structure of the information that is being captured. Derived from neuroscience research, locations on the head are structured according to neuroanatomical planes or axes (Figure \ref{fig:neuralplanes}) and normally the brain, or the head, are divided in different broad regions (Figure \ref{fig:neuroanatomy}).   

\textbf{Spectral Filters}, on the other hand, consider brainwaves as another digital signal, and they perform different transformations based on the spectral information contained within the signal $\mathbf{x}(n)$.  They can be combined creating  \textit{Filter Banks} to enhance signal quality. 
...
\begin{figure}[]
\centering
\includegraphics[height=6cm,width=8cm]{images/emotivsubject.jpg}
\includegraphics[height=6cm,width=8cm]{images/gTecsubject.jpg}
...
\textit{BCI Paradigms} are referred to noninvasive EEG BCI configurations that can be used to transmit volitional information.  They are
...
\section{BCI Algorithms for EEG processing}
