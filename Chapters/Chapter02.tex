...
\item Epoch/Paper speed: the time span that is represented in a single screen.  For paper strips it is usually $10s$.  In its digital counterpart is $ \frac{\lambda}{pixel}$ with $\lambda$ being the span of the signal segment.
\end{enumerate}
...
\caption[EEG Signal Mapping to Images]{Sample EEG signal plotting.  Span of the signal is 1s, which is 250 sample points.  The height of the image is 73 pixels, which is the peak-to-peak amplitude of the signal segment.  Channel Oz is being shown.}
...
There are many ways to regularize the signal, but the \textit{z-score} is widely used and accepted to do this~\cite{Zhang2013}.
...
\caption[Signal Plotting: Interpolation]{A sampled EEG signal interpolated with the Bresenham algorithm.}
\label{fig:interpolation}
\end{figure}

However, as seen in Figure, it can lead to very sharp edges around the sample pixels.  This can lead to a quantization of histogram gradients. The alternative is to use a smoothing interpolated using splines.  Instead of just skipping time points values, these values are interpolated according to a linear quadratic or cubic interpolation, which smooths the curve around each point.  This procedure can be implemented by the Matlab function \textbf{resample} which applies an antialiasing FIR lowpass filter.

\begin{figure}[]
\centering
\includegraphics[scale=10]{images/s_1_e_1_c_7_4.png}
\caption[Signal Plotting: Interpolation]{An interpolated upsampled EEG signal.  Once the new set of sample points are generated, the plot is also interpolated with the Bresenham algorithm.}
\label{fig:imagecoordinatesystem}
\end{figure}
